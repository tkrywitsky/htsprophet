...
def fitInvididualForecast(node_params):
    ...
    changepoints = node_params["changepoints"]
    n_changepoints = node_params["n_changepoints"]
    yearly_seasonality = node_params["yearly_seasonality"]
    weekly_seasonality = node_params["weekly_seasonality"]
    daily_seasonality = node_params["daily_seasonality"]
    holidays = node_params["holidays"]
    seasonality_prior_scale = node_params["seasonality_prior_scale"]
    holidays_prior_scale = node_params["holidays_prior_scale"]
    changepoint_prior_scale = node_params["changepoint_prior_scale"]
    mcmc_samples = node_params["mcmc_samples"]
    interval_width = node_params["interval_width"]
    uncertainty_samples = node_params["uncertainty_samples"]

    h = node_params["h"]
    freq = node_params["freq"]
    include_history = node_params["include_history"]
...
def fitForecast(y, h, sumMat, nodes, method, freq, include_history, cap, capF, changepoints, n_changepoints, \
    ...
    if skipFitting == False:
        ...
        for node_f in node_forecasts:
            node = node_f ["node"]
            forecastsDict[node] = node_f["forecast"]
